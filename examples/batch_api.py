...
class KnockKnock(pydantic.BaseModel):
    ...
    punchline: str = pydantic.Field(description="The punchline for the knock-knock joke")
...
async def agents_example():
    ...
    class KnockKnockAgent(agents.StructuredOutputAgent):
        ...
        def __init__(self, model_name: str | None = None, stopping_condition=None, provider=None, tools=None, callbacks=None, oai_kwargs=None, **fmt_kwargs):
    ...
    print("\n".join(f"Joke {i}:\n{str(KnockKnock.model_validate(res))}" for i, res in enumerate(jokes)))
    ...
    print("The judge crowned a winner!\n\n{}".format(str(KnockKnock.model_validate(jokes[int(best_joke_idx)]))))
